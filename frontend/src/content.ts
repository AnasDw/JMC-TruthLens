...
let highlightedElements: HTMLSpanElement[] = [];
 ...
 */
function setLoadingCursor(): void {
 ...
 */
function removeLoadingCursor(): void {
 ...
 */
function highlightSelectedText(selectedText: string): void {
  try {
    ...
    for (const textNode of textNodes) {
      ...
      if (index !== -1) {
        ...
        try {
          ...
          console.warn(
            "TruthLens: Could not highlight text directly, using fallback method"
          );
        }
      }
    }
 ...
 */
function showVerificationResult(result: VerificationResult): void {
  try {
    highlightedElements.forEach((element) => {
      if (element && element.parentNode) {
        ...
        resultBadge.style.cssText = `
                    ...
                    border: 2px solid ${
                      isTrue ? "#10b981" : isFalse ? "#ef4444" : "#f59e0b"
                    } !important;
        ...
        const details = resultBadge.querySelector(
          ".result-details"
        ) as HTMLElement;
        if (details) {
 ...
 */
function removeTextHighlighting(): void {
  highlightedElements.forEach((element) => {
    try {
      ...
      if (parent) {
        ...
        while (element.firstChild) {
          if (
            (element.firstChild as HTMLElement).className ===
  ...
  const orphanedBadges = document.querySelectorAll(
    ".truthlens-verification-badge"
  );
...
chrome.runtime.onMessage.addListener(
  (
    ...
    if (request.action === "ping") {
      ...
      request.action === "verificationStarted" &&
      request.selectedText
    ) {
      console.log(
        "TruthLens: Verification started for text:",
        request.selectedText
      );
