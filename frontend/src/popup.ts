import type { VerificationResult, TruthLensMessage } from "../types/truthlens";
...
function handlePopupMessage(
  ...
  if (message.action !== "showPopup") return;
  ...
  if (!resultDiv || !statusDot || !popupContainer) return;
 ...
 */
function showErrorState(container: HTMLElement, error: string): void {
 ...
 */
function getVerificationStatus(label: string): string {
 ...
 */
function getSafetyDescription(isSafe?: boolean): string {
  return isSafe
